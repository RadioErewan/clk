...
## loading image zegarxx.png where xx is number of seconds elapsed in current minute
    img = cv.imread('./image2/zegar{:02d}.png'.format(seconds),cv.IMREAD_COLOR)
...
## we are using RGBA wideo frame (including alpha channel)
    ...
    #peek current time
        ...
        #read the clock face
            img = cv.imread('./image2/zegar{:02d}.png'.format(thetime.timetuple()[5]),cv.IMREAD_COLOR)
        ...
        #draw left rundown
            ...
            #draw logo
                third_img = Image.open("./image2/pc{:02d}.png".format(remainingpercent)) 
